...
find_match = function(beforeTokens, afterTokens, indexOfBeforeLocationsInAfterTokens, startInBefore, endInBefore, startInAfter, endInAfter) {
  var bestMatchInAfter = startInAfter,
      bestMatchInBefore = startInBefore,
      indexInBefore = startInBefore,
      i = startInBefore,
      ref = startInBefore,
      bestMatchLength = 0,
      indexInAfter,
  ...
  for (; ref <= endInBefore ? i < endInBefore : i > endInBefore; indexInBefore = ref <= endInBefore ? ++i : --i) {
    newMatchLengthAt = {};
    lookingFor = beforeTokens[indexInBefore];
    locationsInAfter = indexOfBeforeLocationsInAfterTokens[lookingFor];
    for (j = 0, len = locationsInAfter.length; j < len; j = (j + 1)|0) {
...
recursively_find_matching_blocks = function(before_tokens, after_tokens, index_of_before_locations_in_after_tokens, start_in_before, end_in_before, start_in_after, end_in_after, matching_blocks) {
  ...
  if (match != null) {
    ...
    matching_blocks[matching_blocks.length] = match;
...
create_index = function(p) {
  ...
  index = {};
  ref = p.find_these;
  for (i = 0, len = ref.length; i < len; i=(i+1)|0) {
    token = ref[i];
    index[token] = [];
    idx = p.in_these.indexOf(token);
    while (idx !== -1) {
      index[token][index[token].length] = idx;
...
calculate_operations = function(beforeTokens, afterTokens) {
  var action_map = {
        'false,false': 'replace',
        'true,false': 'insert',
        'false,true': 'delete',
      ...
      matches = find_matching_blocks(beforeTokens, afterTokens),
      op,
      operations = [],
      position_in_after = 0,
      position_in_before = 0,
      postProcessed = [];

  if (beforeTokens == null) {
    throw new Error('Before Tokens?');
  }

  if (afterTokens == null) {
    throw new Error('After Tokens?');
  }
  ...
  // };
  // matches = find_matching_blocks(beforeTokens, afterTokens);
  matches[matches.length] = new Match(beforeTokens.length, afterTokens.length, 0);

  for (len = matches.length; i < len; index = ++i) {
    match = matches[index];
    matchStartsAtCurrentPositionInBefore = position_in_before === match.start_in_before;
    matchStartsAtCurrentPositionInAfter = position_in_after === match.start_in_after;
    action_up_to_match_positions = action_map[[matchStartsAtCurrentPositionInBefore, matchStartsAtCurrentPositionInAfter].toString()];
  ...
  //     return false;
  //   }
  //   return /^\s$/.test(beforeTokens.slice(op.start_in_before, +op.end_in_before + 1 || 9e9));
  // };

  for (len1 = operations.length; j < len1; j = (j + 1)|0) {
    op = operations[j];

    if (op.action !== 'equal') {
      postProcessed[postProcessed.length] = op;
      lastOp = op;
      continue;
    }
    ...
    if (((/^\s$/.test(beforeTokens.slice(op.start_in_before, +op.end_in_before + 1 || 9e9))) && lastOp.action === 'replace') || (op.action === 'replace' && lastOp.action === 'replace')) {
...
consecutiveWhere = function(start, content, isnt) {
  var answer,
      index = 0,
      i = 0,
      lastMatchingIndex,
  ...
  if (isnt) {
    for (len = content.length; i < len; index = ++i) {
      // token = content[index];
      answer = /^\s*<[^>]+>\s*$/.test(content[index]);
...
// 		non_tags = consecutiveWhere(position, content, isnt_tag);
...
// 		tags = consecutiveWhere(position, content, is_tag);
...
insWrap = function(content) {
	...
	while (true) {
		...
		non_tags = consecutiveWhere(position, content, false);
		position += non_tags.length;
...
op_map = {
  ...
  insert: function(op, before_tokens, after_tokens) {
    return insWrap(after_tokens.slice(op.start_in_after, +op.end_in_after + 1 || 9e9));
  },
  'delete': function(op, before_tokens, after_tokens) {
    return delWrap(before_tokens.slice(op.start_in_before, +op.end_in_before + 1 || 9e9));
  }
};
...
diff = function(before, after) {
  ...
  return render_operations(before, after, calculate_operations(before, after));
};
